...
const MOCK_PLAYERS = [{name:"JEREMIAS",handicap:18},{name:"DVP",handicap:22},{name:"CRIBBY",handicap:20},{name:"TRANQUILINO",handicap:24},{name:"AL",handicap:8},{name:"FREDDY",handicap:16},{name:"SKIP",handicap:14},{name:"STONEY",handicap:19}]
const MOCK_SCORES:Record<string,number[]> = {JEREMIAS:[6,5,4,4,5,4,5,5,5,5,3,4,5,5,4,3,5,7],DVP:[6,4,6,5,5,4,5,4,6,6,4,6,6,6,4,3,5,5],CRIBBY:[4,6,5,5,6,4,6,4,5,5,3,6,4,5,4,4,5,7],TRANQUILINO:[6,6,4,4,6,4,5,5,6,6,4,5,5,5,5,4,4,6],AL:[4,6,4,3,6,4,4,4,4,5,3,4,6,4,5,2,6,6],FREDDY:[6,6,4,5,4,4,5,4,5,4,3,5,6,4,4,3,5,7],SKIP:[5,7,5,4,5,3,5,5,5,4,3,5,4,5,5,3,6,5],STONEY:[6,6,6,4,4,4,4,5,6,5,4,4,6,5,4,3,4,5]}
const MOCK_TEAMS = [{name:"Team 1",players:["JEREMIAS","DVP","CRIBBY","TRANQUILINO"]},{name:"Team 2",players:["AL","FREDDY","SKIP","STONEY"]}]
...
export default function AdminWizard() {
  ...
  useEffect(() => {
    ...
    onValue(ref(db,'tournament/format'), snap => {
      if (snap.val()) { setFormatCustom(snap.val().name !== "Jeff's Blitz"); setFormatName(snap.val().name || "Jeff's Blitz") }
    })
    ...
    // Check which days are archived for this trip
    onValue(ref(db,'history'), snap => {
      if (!snap.val()) { setArchivedDays([]); return }
      const days: string[] = []
      Object.values(snap.val()).forEach((h:any) => {
        if (h._meta?.dayLabel) days.push(h._meta.dayLabel)
      })
  ...
  const saveTripMeta = async () => {
    await set(ref(db,'tournament/meta'), { ...meta, tripName: tripNameInput.trim(), totalDays: totalDaysInput, currentDay: meta.currentDay || 'Day 1', isMock: meta.isMock || false })
  ...
  const startNextDay = async () => {
    ...
    if (!confirm(`START ${nextDay.toUpperCase()}? This will clear scores and matchups. Teams and course stay.`)) return
    setTransitioningDay(true)
    // Archive current day first
    const snap = await get(ref(db,'tournament'))
    if (snap.exists()) {
    ...
    // Wipe scores and matchups, keep everything else
    ...
    flash(`✓ Day archived. Now setting up ${nextDay} — configure your matchups then go live.`)
    setTransitioningDay(false)
  }

  const loadMock = async () => {
    if (!confirm("LOAD MOCK TOURNAMENT?")) return
    setLoading(true)
    await set(ref(db,'tournament'), null)
    ...
    for (const p of MOCK_PLAYERS) { const pRef=push(ref(db,'tournament/roster')); await set(pRef,{id:pRef.key,...p}); playerIdMap[p.name]=pRef.key! }
    for (const t of MOCK_TEAMS) { const tRef=push(ref(db,'tournament/teams')); await set(tRef,{id:tRef.key,name:t.name,playerIds:t.players.map(n=>playerIdMap[n])}) }
    const sd:Record<string,number[]>={};
    for (const [n,hs] of Object.entries(MOCK_SCORES)) { const pid=playerIdMap[n]; if(pid) sd[pid]=hs }
    await set(ref(db,'tournament/scores'),sd)
    const pvp=push(ref(db,'tournament/matchups')); await set(pvp,{id:pvp.key,type:'PvP',sideA:'AL',sideB:'FREDDY',nassau:5,press:5,birdie:2,eagle:5,scoringType:'NET',autoPress:true})
    const tvt=push(ref(db,'tournament/matchups')); await set(tvt,{id:tvt.key,type:'TvT',sideA:'Team 1',sideB:'Team 2',nassau:10,press:10,birdie:2,eagle:5,scoringType:'NET'})
    await set(ref(db,'tournament/money'),{entryFee:25,skinsAllocation:10})
    flash("✓ Mock tournament loaded.")
    setLoading(false)
  }
    ...
    <div className="min-h-screen bg-black text-white font-sans uppercase italic">
      ...
      <div className="max-w-lg mx-auto px-4 py-8 space-y-3">
        ...
        {hasAnyData && (
          <div className={`rounded-2xl border-2 p-5 ${meta.isMock ? 'border-amber-500/40 bg-amber-500/5' : 'border-blue-500/30 bg-blue-500/5'}`}>
            ...
            <div className="flex gap-2">
              {meta.isMock ? (
                <button onClick={()=>clearData('mock')} disabled={loading} className="flex-1 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-400 py-3 px-4 rounded-xl font-black text-xs flex items-center justify-center gap-2 transition-all">
                  <Trash2 size={12}/> CLEAR MOCK DATA
                </button>
              ) : (
                <button onClick={()=>clearData('archive')} disabled={loading} className="flex-1 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/40 text-blue-400 py-3 px-4 rounded-xl font-black text-xs flex items-center justify-center gap-2 transition-all">
                  <Archive size={12}/> ARCHIVE + FULL RESET
                </button>
              )}
            </div>
          </div>
        ...
        >
          {editingTrip ? (
            ...
            <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {Array.from({length: meta.totalDays}, (_,i) => {
                      ...
                      return (
                        <div key={i} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border font-black text-xs transition-all ${
                          ...
                          {dayLabel}
                          {isArchived && <span className="text-[9px]">✓</span>}
              ...
              {/* Next day transition button */}
              {canGoNextDay && (
                <button
                  onClick={startNextDay}
                  disabled={transitioningDay}
        ...
        <div className="pt-4 border-t border-zinc-900">
          ...
          <button onClick={loadMock} disabled={loading} className="w-full bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 hover:border-zinc-500 py-3 px-5 rounded-2xl font-black text-sm text-zinc-400 hover:text-white flex items-center justify-between transition-all">
            <span className="flex items-center gap-3">{loading?<Loader2 size={16} className="animate-spin"/>:<FlaskConical size={16} className="text-zinc-500"/>} LOAD MOCK TOURNAMENT</span>
            <span className="text-[9px] text-zinc-600 normal-case">8 players · full scores</span>
          </button>
        </div>
...
function StepCard({number,title,summary,status,icon,children}:{number:number;title:string;summary:string;status:'complete'|'empty'|'warning';icon:React.ReactNode;children:React.ReactNode}) {
  const isComplete = status==='complete'; const isWarning = status==='warning'
  return (
