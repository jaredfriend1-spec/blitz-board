...
return (
 ...
 <div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-2">MATCH TYPE</label><div className="flex gap-2"><button onClick={() => setNewMatchType('PvP')} className={`flex-1 py-2 rounded-lg font-bold text-sm transition-all ${newMatchType === 'PvP' ? 'bg-blue-500 text-white border-blue-400' : 'bg-zinc-800 text-zinc-400 border-zinc-700'} border`}>1v1</button><button onClick={() => setNewMatchType('2v2')} className={`flex-1 py-2 rounded-lg font-bold text-sm transition-all ${newMatchType === '2v2' ? 'bg-blue-500 text-white border-blue-400' : 'bg-zinc-800 text-zinc-400 border-zinc-700'} border`}>2v2</button></div></div>
 {(() => { const arch = archives.find(a => a.id === addMatchupsTo); const players = arch?.roster ? Object.values(arch.roster).map((p: any) => p.name) : []; return (<>{newMatchType === 'PvP' ? (<><div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-2">PLAYER A</label><select value={newMatchData.sideA} onChange={(e) => setNewMatchData({...newMatchData, sideA: e.target.value})} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-2 text-white font-bold text-sm"><option value="">SELECT</option>{players.map(name => <option key={name} value={name}>{name}</option>)}</select></div><div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-2">PLAYER B</label><select value={newMatchData.sideB} onChange={(e) => setNewMatchData({...newMatchData, sideB: e.target.value})} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-2 text-white font-bold text-sm"><option value="">SELECT</option>{players.map(name => <option key={name} value={name}>{name}</option>)}</select></div></>) : (<><div className="grid grid-cols-2 gap-2"><div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-1">TEAM A P1</label><select value={newMatchData.sideA} onChange={(e) => setNewMatchData({...newMatchData, sideA: e.target.value})} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-2 text-white font-bold text-xs"><option value="">SELECT</option>{players.map(name => <option key={name} value={name}>{name}</option>)}</select></div><div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-1">TEAM A P2</label><select value={newMatchData.sideA2} onChange={(e) => setNewMatchData({...newMatchData, sideA2: e.target.value})} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-2 text-white font-bold text-xs"><option value="">SELECT</option>{players.map(name => <option key={name} value={name}>{name}</option>)}</select></div></div><div className="grid grid-cols-2 gap-2"><div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-1">TEAM B P1</label><select value={newMatchData.sideB} onChange={(e) => setNewMatchData({...newMatchData, sideB: e.target.value})} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-2 text-white font-bold text-xs"><option value="">SELECT</option>{players.map(name => <option key={name} value={name}>{name}</option>)}</select></div><div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-1">TEAM B P2</label><select value={newMatchData.sideB2} onChange={(e) => setNewMatchData({...newMatchData, sideB2: e.target.value})} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-2 text-white font-bold text-xs"><option value="">SELECT</option>{players.map(name => <option key={name} value={name}>{name}</option>)}</select></div></div></>)}</>); })()}
 <div className="grid grid-cols-2 gap-3"><div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-1">NASSAU</label><input type="number" value={newMatchData.nassau} onChange={(e) => setNewMatchData({...newMatchData, nassau: Number(e.target.value)})} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-2 text-white font-bold text-sm" /></div><div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-1">PRESS</label><input type="number" value={newMatchData.press} onChange={(e) => setNewMatchData({...newMatchData, press: Number(e.target.value)})} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-2 text-white font-bold text-sm" /></div></div>
 {newMatchType === 'PvP' && <div className="grid grid-cols-2 gap-3"><div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-1">BIRDIE</label><input type="number" value={newMatchData.birdie} onChange={(e) => setNewMatchData({...newMatchData, birdie: Number(e.target.value)})} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-2 text-white font-bold text-sm" /></div><div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-1">EAGLE</label><input type="number" value={newMatchData.eagle} onChange={(e) => setNewMatchData({...newMatchData, eagle: Number(e.target.value)})} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-2 text-white font-bold text-sm" /></div></div>}
 <div><label className="text-xs font-black text-zinc-400 tracking-widest block mb-2">SCORING</label><div className="flex gap-2"><button onClick={() => setNewMatchData({...newMatchData, scoringType: 'NET'})} className={`flex-1 py-2 rounded-lg font-bold text-sm transition-all ${newMatchData.scoringType === 'NET' ? 'bg-emerald-600 text-white' : 'bg-zinc-800 text-zinc-400'} border border-current`}>NET</button><button onClick={() => setNewMatchData({...newMatchData, scoringType: 'GROSS'})} className={`flex-1 py-2 rounded-lg font-bold text-sm transition-all ${newMatchData.scoringType === 'GROSS' ? 'bg-emerald-600 text-white' : 'bg-zinc-800 text-zinc-400'} border border-current`}>GROSS</button></div></div>
 <label className="flex items-center gap-3 cursor-pointer bg-zinc-800 p-3 rounded-lg"><input type="checkbox" checked={newMatchData.autoPress} onChange={(e) => setNewMatchData({...newMatchData, autoPress: e.target.checked})} className="w-4 h-4" /><span className="text-sm font-bold text-zinc-300">Auto Press Enabled</span></label>
 <div className="flex gap-3 pt-4"><button onClick={() => { setAddMatchupsTo(null); setNewMatchData({ sideA: '', sideB: '', sideA2: '', sideB2: '', nassau: 5, press: 5, birdie: 2, eagle: 5, scoringType: 'NET', autoPress: true }) }} className="flex-1 py-2 rounded-lg font-bold text-sm bg-zinc-800 hover:bg-zinc-700 text-zinc-300 border border-zinc-700 transition-all">Cancel</button><button onClick={saveNewMatchup} className="flex-1 py-2 rounded-lg font-bold text-sm bg-blue-600 hover:bg-blue-500 text-white border border-blue-500 transition-all">Save Matchup</button></div>
 </div>
 </div>
 )}
 )
