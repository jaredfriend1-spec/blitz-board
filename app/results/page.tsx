...
import { Trophy, Award, ArrowLeft, LayoutGrid } from 'lucide-react'
import Link from 'next/link'

export default function ResultsPage() {
  const [scores, setScores] = useState<Record<string, number[]>>({})
  const [players, setPlayers] = useState<any[]>([])
  const [money, setMoney] = useState({ stakeAmount: 20 })
  ...
  const getResults = () => {
    ...
    // Skins Map (Hole by Hole)
    const map = Array(18).fill(null);
    const skinsCount: Record<string, number> = {}
    for (let h = 0; h < 18; h++) {
      const holeScores = players.map(p => ({ id: p.id, name: p.name, s: (scores[p.id] || [])[h] || 0 })).filter(x => x.s > 0)
    ...
    <div className="min-h-screen bg-black text-white p-6 font-sans uppercase italic">
      ...
      <section className="max-w-6xl mx-auto mb-12">
        <div className="bg-emerald-500 text-black p-6 rounded-t-[2.5rem] border-x-4 border-t-4 border-black flex items-center gap-3 justify-center">
          <LayoutGrid size={24} /><h2 className="text-3xl font-black">Skins Dashboard</h2>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        ...
        <section className="bg-orange-100 rounded-[3rem] border-4 border-black overflow-hidden">
          <div className="bg-orange-500 p-6 font-black text-center text-white text-xl border-b-4 border-black">SKINS WINNINGS</div>
          <table className="w-full text-black font-black text-xs">
            ...
            <tbody>{players.map(p => (
              <tr key={p.id} className="border-b border-orange-200">
                <td className="p-5">{p.name}</td><td className="p-5 text-center text-lg">{res.skinsCount[p.id] || 0}</td>
                <td className="p-5 text-right text-emerald-700 text-lg">${(res.skinsCount[p.id] || 0) * money.stakeAmount}</td>
              </tr>
            ))}</tbody>
          </table>
        </section>
