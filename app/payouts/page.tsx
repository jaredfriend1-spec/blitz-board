...
// ── OPTIMAL BALL ASSIGNMENT ENGINE ────────────────────────────────
// Find the best combination of player scores for a hole.
// Rules:
//   - Use exactly `balls.length` players
//   - Each player contributes exactly ONE score (net OR gross, not both)
//   - Minimize the total
// ──────────────────────────────────────────────────────────────────
...
function getBestTeamScore(players: PlayerHoleScore[], balls: Ball[]): number {
  // Only players who actually played the hole
  ...
  // Try all subsets of `ballCount` players from available
  for (const subset of combinations(available, ballCount)) {
    // From this subset, try all ways to assign `grossCount` players to gross roles
    for (const grossSubset of combinations(subset, grossCount)) {
      ...
      const total =
        grossSubset.reduce((sum, p) => sum + p.gross, 0) +
        netSubset.reduce((sum, p) => sum + p.net, 0)
...
export default function PayoutsPage() {
  ...
  useEffect(() => {
    ...
    onValue(ref(db, 'tournament/format'), snap => {
      if (snap.val()) setFormat(snap.val())
      else setFormat(JEFFS_BLITZ) // default if not configured
    })
  }, [])

  const calculateMatch = (m: any) => {
    const pA = m.type === 'PvP'
      ? players.filter(p => p.name === m.sideA)
      : players.filter(p => (teams.find(t => t.name === m.sideA)?.playerIds || []).includes(p.id))
    const pB = m.type === 'PvP'
      ? players.filter(p => p.name === m.sideB)
      : players.filter(p => (teams.find(t => t.name === m.sideB)?.playerIds || []).includes(p.id))
    ...
    for (let i = 0; i < 18; i++) {
      ...
      // Build PlayerHoleScore arrays for each side
      ...
      if (m.type === 'PvP') {
        // PvP: best single score (net or gross based on scoringType)
        ...
        // TvT: use configured format — optimal ball assignment engine
        ...
        // Birdie/eagle (best gross per team)
    ...
    const runNine = (start: number, end: number) => {
      ...
      for (let i = start; i <= end; i++) {
        ...
        if (delta !== 0) {
          bets.forEach(b => {
            b.score += delta
            if (m.type === 'PvP' && useAutoPress && Math.abs(b.score) >= 2 && !b.pressed) {
              b.pressed = true; newP++; totalP++
            }
          })
          if (m.type === 'PvP' && useAutoPress) {
    ...
    return { sA_net: sA_final, sB_net: sB_final, sA_dots, sB_dots, holeBonus, strokesA, strokesB, f9, b9, birdieA, birdieB, net, useAutoPress }
  }
          ...
          <table className="w-full text-center">
            ...
            <tbody>
              <tr className="border-t border-zinc-900">
                <td className="py-3 px-4 text-left text-emerald-400 font-black text-sm truncate max-w-[7rem]">{m.sideA}</td>
                {holes.map(i => (
              ...
              <tr className="border-t border-zinc-900 bg-white/[0.02]">
                <td className="py-3 px-4 text-left text-blue-400 font-black text-sm truncate max-w-[7rem]">{m.sideB}</td>
                {holes.map(i => (
                  <td key={i} className="py-2 px-0.5">
                  ...
                  <td key={idx} className="py-2 px-0.5 relative">
                    ...
                    {h.bonus && (
                      <div className="flex justify-center mt-0.5">
                        {h.bonus.type === 'eagle'
                          ? <span title={`Eagle — ${h.bonus.side === 'A' ? m.sideA : m.sideB}`} className="text-yellow-400 text-[10px] leading-none">★</span>
                          : <div title={`Birdie — ${h.bonus.side === 'A' ? m.sideA : m.sideB}`} className={`w-2 h-2 rounded-full ${h.bonus.side === 'A' ? 'bg-emerald-500' : 'bg-blue-500'}`}/>
                        }
                      </div>
                    )}
                  </td>
  ...
  // Format summary for match header
  const formatLabel = (m: any) => {
    if (m.type !== 'TvT') return null
    return format.name || "Jeff's Blitz"
  }
...
>>>>>>> 53af54a1adce0e299d776e83d6cb7dadec2e145f
      ...
      <div className="max-w-4xl mx-auto mb-6 flex items-center gap-3 bg-zinc-900/50 border border-zinc-800 rounded-2xl px-4 py-3">
        <span className="text-[10px] font-black text-zinc-600 tracking-widest">TEAM FORMAT</span>
        <span className="text-emerald-400 font-black text-sm">{format.name}</span>
        <span className="text-zinc-700 text-[10px] font-black">·</span>
        <span className="text-zinc-500 text-[10px] font-black normal-case">
          Par 3: {format.par3?.length || 3}B · Par 4: {format.par4?.length || 2}B · Par 5: {format.par5?.length || 2}B
        </span>
      </div>
      ...
      <div className="max-w-4xl mx-auto mb-6 flex items-center gap-5 flex-wrap text-[10px] font-black text-zinc-600 tracking-widest">
        ...
        <span className="flex items-center gap-1.5"><span className="text-yellow-400 text-sm leading-none">★</span> EAGLE BONUS</span>
            ...
            <div key={m.id} className="bg-zinc-950 rounded-[3rem] border-2 border-zinc-800 shadow-2xl overflow-hidden">
              <div className="p-6 sm:p-8 border-b-2 border-zinc-900">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {m.type === 'TvT' && (
                    <span className="px-3 py-1.5 rounded-lg text-xs font-black bg-purple-500/20 text-purple-400">
                      {formatLabel(m)}
                    </span>
                  )}
                  {m.type === 'PvP' && (
                    <span className={`px-3 py-1.5 rounded-lg text-xs font-black flex items-center gap-1 ${
                      res.useAutoPress ? 'bg-yellow-500/20 text-yellow-400' : 'bg-zinc-800 text-zinc-500'
                    }`}>
                  ...
                  <span className="bg-zinc-900 px-3 py-1.5 rounded-lg text-xs font-black text-blue-400">Birdie ${m.birdie} · Eagle ${m.eagle || m.birdie * 2}</span>
                </div>
              </div>
