...
export default function PayoutsPage() {
  ...
  const [money, setMoney] = useState({ sideBetAmount: 17 })
  ...
  return (
    <div className="min-h-screen bg-black text-white p-8 font-sans uppercase italic">
      <Link href="/" className="text-emerald-500 font-black mb-12 inline-block"><ArrowLeft size={18} className="inline mr-2" /> HUB</Link>
      
      <div className="max-w-6xl mx-auto space-y-16">
        {matches.map(m => {
          const pA = players.find(x => x.name === m.sideA);
          const pB = players.find(x => x.name === m.sideB);
          if (!pA || !pB) return null; // Wait until valid players are mapped
                ...
                <table className="w-full text-center border-collapse">
                  ...
                  <tbody className="text-[12px] font-black">
                    <tr className="border-t border-zinc-900">
                      <td className="p-4 text-left text-emerald-500 truncate">{m.sideA}</td>
                      {sA.map((s: number, i: number) => <td key={i} className={`p-2 ${s > 0 ? (s < course.pars[i] ? 'text-red-500' : 'text-emerald-400') : 'text-zinc-800'}`}>{s || '-'}</td>)}
                    </tr>
                    <tr className="border-t border-zinc-900">
                      <td className="p-4 text-left text-emerald-500 truncate">{m.sideB}</td>
                      {sB.map((s: number, i: number) => <td key={i} className={`p-2 ${s > 0 ? (s < course.pars[i] ? 'text-red-500' : 'text-emerald-400') : 'text-zinc-800'}`}>{s || '-'}</td>)}
                    </tr>
                    <tr className="border-t-2 border-zinc-800 bg-zinc-900/50">
                      <td className="p-4 text-left text-zinc-600">WINNER</td>
                      {Array.from({length:18}).map((_, i) => {
                        ...
                        if (isPress) presses++;
                        return <td key={i} className={`p-2 ${winner === 'A' ? 'text-emerald-500' : winner === 'B' ? 'text-amber-500' : 'text-zinc-600'}`}>
                          {winner} {isPress && <Zap size={10} className="text-yellow-500 inline"/>}
                        </td>
                      })}
                    </tr>
                  </tbody>
