export const golfers = [
  { id: '1', name: 'JEFF PERKINS', handicap: 0 },
  { id: '2', name: 'RICK SOVERO', handicap: 0 },
  { id: '3', name: 'SAM OSHRINE', handicap: 0 },
  { id: '4', name: 'TONY ZAMOSTNY', handicap: 0 },
  { id: '5', name: 'ANDREW SOVERO', handicap: 0 },
  { id: '6', name: 'SAM OSHRINE', handicap: 0 },
  { id: '7', name: 'IRA COHEN', handicap: 0 },
  { id: '8', name: 'JOSH BUMGARNER', handicap: 0 },
];

export const BLITZ_TEAMS = [
  { id: 't1', name: 'TEAM 1', playerIds: ['1', '2', '3', '4'] },
  { id: 't2', name: 'TEAM 2', playerIds: ['5', '6', '7', '8'] },
];

export const tournamentSettings = { 
  name: "Blitz Board", 
  date: "04/28/26" 
};